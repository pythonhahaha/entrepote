...
class DjangoMigrations(models.Model):
    ...
    class Meta:
        ...
        verbose_name_plural = u'django迁移'
...
class UserInfo(models.Model):
    ...
    def __unicode__(self):
        return 'UserInfo:%s'%(self.is_used)

    class Meta:
        managed = False
        db_table = 'user_info'
        verbose_name_plural = u'添加员工'
...
class UserRole(models.Model):
    ...
    is_used = models.CharField(max_length=10, blank=True, null=True)
    def __unicode__(self):
        return 'UserRole:%s%s%s'%(self.role_name,self.role_power,self.is_used)

    class Meta:
        managed = False
        db_table = 'user_role'
        verbose_name_plural = u'部门分类'
